...
try {
  const rawOut = execSync('npx vitest run --reporter=default', {
  ...
  if (filesMatch && testsMatch && skillTestMatch) {
    ...
    console.log('⚠️  Could not parse test output');
  }
} catch (e) {
  // execSync throws on non-zero exit OR if stderr has content; check stdout anyway
