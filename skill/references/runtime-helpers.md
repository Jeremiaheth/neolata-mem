# Runtime Helpers — `src/runtime.mjs`
...
**Channels:**
1. **Recency** — latest memories (no rerank)
2. **Semantic** — similarity to `seedText` (reranked)
3. **Importance** — high-importance memories matching topic slug (filtered by threshold)
...
| Param | Type | Default | Description |
|-------|------|---------|-------------|
| `config.sessionId` | `string?` | — | Session tag |
| `config.topicSlug` | `string?` | — | Topic tag |
| `config.projectSlug` | `string?` | — | Project tag |
